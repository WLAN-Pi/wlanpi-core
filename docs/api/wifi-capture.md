...
**Status:** Design, not implemented. No REST capture API exists yet.  
**Version:** 2026.06.14  
**Audience:** wlanpi-core implementers, MCP tool authors, OpenAPI maintainers

> **Not implemented.** The capture interface that ships today is the WebSocket
> contract in [capture-ws-mcp-handover.md](../capture-ws-mcp-handover.md). This
> document designs a future REST capture API.

**Related:**
- [Application developer guide](./wifi-capture-consumer-guide.md) — how to build controllers and subscribers
- [Core worker API](../core-worker-api.md) §2.4.1, Stream D
- [WLAN scan API](./wlan-scan.md) — snapshot scan (not continuous capture)
- [Gap matrix](../api-gap-matrix.csv)
- **Shipped today:** `WS /api/v1/streaming/capture`, described in [capture-ws-mcp-handover.md](../capture-ws-mcp-handover.md)
...
**Design rule:** REST orchestrates jobs; WebSocket carries **live pcap bytes** and **low-latency events**. MCP uses **summary mode only** — no WebSocket required for AI tools.

---

## 3. Capture modes

| Mode | Scope | Data path | Primary consumer |
|------|-----|-----------|------------------|
...
| **`file`** | Optional | Written pcap on disk; `GET .../files` | Long captures, post-processing |

Session `mode` is set at create time. MCP tools use **`summary` only**.
...
BPF is compiled **server-side** from structured filter — clients do not send raw BPF strings.
...
Control commands on WebSocket (`start`, `stop`, `configure`) are **rejected** for subscribe tokens — control is REST-only.
...
- **Independent hop loops** per source with per-source `filter` override (later; shared filter initially).
...
MCP **does not** open WebSocket. Stream mode is for non-MCP application developers.
