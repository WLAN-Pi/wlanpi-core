...
**Core design (implementers):** [wifi-capture.md](./wifi-capture.md)  
**Related:** [WLAN scan API](./wlan-scan.md) (one-shot scan, not live capture)

> **Not implemented.** For the capture interface that ships today, see
> [capture-ws-mcp-handover.md](../capture-ws-mcp-handover.md).
...
MCP tools use **summary mode only** — no WebSocket.
...
OpenAPI + MCP tool names are listed in [wifi-capture.md §10](./wifi-capture.md#10-mcp-tool-mapping).
...
This guide describes the **planned REST capture API**, which is not implemented. The shipped interface is the authenticated WebSocket `WS /api/v1/streaming/capture` described in [capture-ws-mcp-handover.md](../capture-ws-mcp-handover.md). Check the [gap matrix](../api-gap-matrix.csv) for `Live` status.
