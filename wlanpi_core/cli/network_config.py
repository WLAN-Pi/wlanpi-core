...
class NetworkConfigCLI:
        ...
        try:
            ...
            headers = {"Authorization": f"Bearer {self.token}", 
                       "accept": "application/json",
                        ...
                        "X-Request-Signature": signature} if self.token else {}
    ...
    def get_token(self):
        getjwt_output = subprocess.run(
            ["getjwt", "network_config_cli", "--port", str(self.API_PORT), "--no-color"],
            capture_output=True,
            text=True,
            check=True
        ).stdout
    ...
    def new_config(self):
        ...
        use_namespace = input("Use namespace? (y/n, default 'n'): ").strip().lower() == 'y'
        mode = input("Enter mode (managed/monitor, default 'managed'): ").strip() or "managed"
        iface_display_name = input("Enter interface display name (default 'wlanpi0'): ").strip() or "wlanpi0"
        ...
        security = input("Enter security type (only supports WPA2-PSK currently): ").strip()
        ...
        default_route = input("Set as default route? (y/n, default 'n'): ").strip().lower() == 'y'
        app_id = input("Enter app to autostart (optional, configured in next step): ").strip()
        if app_id:
    ...
    def activate_config(self):
        ...
        if config_active:
            ...
            override = input("Override and activate anyway? (y/n): ").strip().lower() == 'y'
            if not override:
                print("Activation cancelled.")
                return
        print(f"Activating configuration {config_id}... (~30s)")
        response = self.post(self.BASE + f"activate/{config_id}?override_active={'true' if override else 'false'}", {})
    ...
    def deactivate_config(self):
        ...
        if not config_active:
            ...
            override = input("Override and deactivate anyway? (y/n): ").strip().lower() == 'y'
            if not override:
                print("Deactivation cancelled.")
                return
        print(f"Deactivating configuration {config_id}...")
        response = self.post(self.BASE + f"deactivate/{config_id}?override_active={'true' if override else 'false'}", {})
    ...
    def delete_config(self):
        ...
        if config_active:
            ...
            override = input("Override and delete anyway? (y/n): ").strip().lower() == 'y'
            if not override:
                print("Deletion cancelled.")
                return
        print(f"Deleting configuration {config_id}...")
        response = self.delete(self.BASE + f"{config_id}?force={'true' if override else 'false'}", {})
        print(f"Configuration {config_id} deactivated successfully.")
