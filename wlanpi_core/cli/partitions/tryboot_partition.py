...
def confirm_tryboot() -> bool:
    """Confirm with user before setting up tryboot to the alternate partition."""
    print(
        "\nThis will configure the system to boot into the alternate partition set once."
    )
    print(
        "The system will reboot after this operation if '--no-reboot' is not specified."
    )
    print(
        "If the alternate partition fails to boot, the system will revert to the current partition on the next reboot."
    )
...
def tryboot_alternate_partition(
    no_confirm: bool = False, no_reboot: bool = False
) -> int:
    """
    Configure the system to try booting into the alternate partition set.
    ...
    try:
        ...
        if not lock_service.acquire_lock("tryboot", "cli"):
            echo_error("Failed to acquire lock for tryboot operation.")
            return 1
        ...
        try:
            ...
            if current_set not in ["A", "B"]:
                echo_warning(
                    f"Current partition state is '{current_set}', which indicates a potential issue.\n"
                    f"Trying to boot into the alternate partition may not work as expected."
                )
                if not no_confirm:
            ...
            if not no_confirm and not confirm_tryboot():
                echo_status("Operation cancelled.")
                return 0

            current_boot_device = partition_service.get_device_paths()[
                "boot1" if current_set == "A" else "boot2"
            ]

            echo_status(f"Setting up one-time boot to partition set {alternate_set}")
            success = config_service.create_tryboot_txt(
                current_boot_device, target_boot_num
            )

            if not success:
                echo_error(
                    "Failed to create tryboot configuration. The operation did not complete successfully."
                )
                return 1
            ...
            if not no_reboot:
                ...
                echo_debug(
                    "The system will automatically use the current partition on subsequent boots."
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Configure the system to try booting into the alternate partition set."
    )
    parser.add_argument(
    ...
    return tryboot_alternate_partition(
        no_confirm=args.no_confirm, no_reboot=args.no_reboot
    )
