...
class TestConfigService:
    ...
    def config_service(self, mock_device_service, mock_partition_service):
        ...
        with patch("wlanpi_core.services.partitions.config_service.DeviceService", 
                  return_value=mock_device_service), \
             patch("wlanpi_core.services.partitions.config_service.PartitionService", 
    ...
    @patch("builtins.open", new_callable=unittest.mock.mock_open, read_data="console=tty1 root=PARTUUID=12345678-01 rootwait")
    def test_update_cmdline_txt(self, mock_open, mock_exists, mock_mkdtemp, config_service, mock_partition_service):
        ...
        handle.write.assert_called_once_with("console=tty1 root=PARTUUID=87654321-01 rootwait")

    @patch("wlanpi_core.services.partitions.config_service.tempfile.mkdtemp")
    @patch("wlanpi_core.services.partitions.config_service.os.path.exists")
    @patch("builtins.open", new_callable=unittest.mock.mock_open)
    def test_create_tryboot_txt(self, mock_open, mock_exists, mock_mkdtemp, config_service, mock_partition_service):
    ...
    def test_tryboot_alternate_partition(self, mock_run_command, config_service, mock_partition_service):
    ...
    @patch("wlanpi_core.services.partitions.config_service.tempfile.mkdtemp")
    @patch("wlanpi_core.services.partitions.config_service.os.path.exists")
    @patch("builtins.open", new_callable=unittest.mock.mock_open, read_data="PARTUUID=12345678-07 /home    ext4    defaults,noatime 0 2")
    def test_update_fstab(self, mock_open, mock_exists, mock_mkdtemp, config_service, mock_partition_service):
        ...
        handle.write.assert_called_once_with("PARTUUID=87654321-07 /home    ext4    defaults,noatime 0 2")
    ...
    def test_prepare_partition_update(self, config_service, mock_partition_service):
        ...
        mock_partition_service.get_set_paths.return_value = {"boot": "/dev/mmcblk0p2", "root": "/dev/mmcblk0p6"}
