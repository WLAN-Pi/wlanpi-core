...
class VLANFile:
    STANZA_PREFIXES = (
    "iface", "mapping", "auto", "allow-hotplug", "allow-auto", "rename", "source", "source-directory")
    ...
    def __init__(self, interface_file: str =DEFAULT_INTERFACE_FILE, vlan_interface_file: str =DEFAULT_VLAN_INTERFACE_FILE):
        self.vlans = []
    ...
    def get_vlans(self, interface: Optional[str] = None) -> list:
        ...
        vlans_devices: dict[str, dict[str, any]] = defaultdict(lambda: defaultdict(lambda: {
            'addresses': [],
            'selection': None,
        }))
    ...
    async def create_update_vlan(self, configuration: Vlan, require_existing_interface: bool = True):
        """
        Creates or updates a VLAN definition for a given interface.
        """

        # Validate that the requested interface exists
        if require_existing_interface and not await self.check_interface_exists(configuration.interface):
            # return {
            #     'success': False,
            #     'result': self.vlans,
        ...
        for vlan_index, existing_vlan in enumerate(self.vlans):
            ...
            if existing_vlan['interface'] == configuration.interface and str(existing_vlan['vlan_tag']) == str(
                    configuration.vlan_tag):
    ...
    async def remove_vlan(self, interface: str, vlan_tag: Union[str, int], allow_missing=False):
        """
        ...
        self.vlans = [ i for i in self.vlans if not (i['interface'] == interface and str(i['vlan_tag']) == str(
                    vlan_tag)) ]
    ...
    def persist_vlans(self):
        output_string = '\n'.join(
            map(lambda f: self.generate_if_config_from_object(Vlan.model_validate(f)), self.vlans))
