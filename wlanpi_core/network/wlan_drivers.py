...
def _bus_for_interface(iface: str) -> str | None:
    try:
        ...
        if not match:
            return None
        wiphy = match.group(1)
        for line in run_command(["ls", "-l", f"/sys/class/ieee80211/phy{wiphy}/device"]).stdout.splitlines():
            if "usb" in line:
                return "usb"
            if "pci" in line:
                return "pci"
...
def get_pci_wlan_drivers() -> dict[str, Any]:
    ...
    for iface in interfaces:
        if _bus_for_interface(iface) != "pci":
            continue
        adapters.append(
            {
                "interface": iface,
    ...
    return {"adapters": adapters, "pci_devices": pci_devices}
